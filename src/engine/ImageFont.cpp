...
{
	CharDataMap::const_iterator anItr = theFontLayer.mCharDataMap.begin();
	while (anItr != theFontLayer.mCharDataMap.end())
	{
...
CharData* FontLayer::GetCharData(uint32_t theChar)
{
...
static uint32_t UTF8CharToUTF32Char(const std::string &theString)
{
	uint32_t aChar;
	size_t aOffset = 0;
...
static bool UTF8PairToUTF32Pair(const std::string& theString, uint32_t& firstChar, uint32_t& secondChar)
{
	size_t aOffset = 0;
		...
		if (theParams.mElementVector.size() == 4)
		{
				...
				(DataToIntVector(theParams.mElementVector[3], &aCharWidthsVector)))
			{
				if (aCharsVector.size() == aCharWidthsVector.size())
				{
					...
					{
						uint32_t first_char = UTF8CharToUTF32Char(aCharsVector[i]);
					...
					if ((Image*)aLayer->mImage != NULL)
					{
						...
						for (uint32_t i = 0; i < aCharsVector.size(); i++)
						{
...

							{
								...
								{
									char aStrBuf[512];
									snprintf(aStrBuf, sizeof(aStrBuf), "Image rectangle out of bounds %d %d %d %d - %d %d", aRect.mX, aRect.mY, aRect.mWidth, aRect.mHeight, anImageWidth, anImageHeight);
									Error(aStrBuf);
									return false;
								}
						...
						CharDataMap::iterator aCDItr = aLayer->mCharDataMap.begin();
						while (aCDItr != aLayer->mCharDataMap.end())
						{
							CharData& aCharData = aCDItr->second;
...
bool FontData::LoadLegacy(Image* theFontImage, const std::string& theFontDescFileName)
{
	...
	FILE* aStream = fopen(aFilePath.c_str(), "r");
	...
	mOwnsImage(theActiveFontLayer.mOwnsImage)
{
	...
	CharRectMap::const_iterator aCRItr = theActiveFontLayer.mScaledCharImageRects.begin();
	while (aCRItr != theActiveFontLayer.mScaledCharImageRects.end())
	{
		mScaledCharImageRects.insert(CharRectMap::value_type(aCRItr->first, aCRItr->second));
		++aCRItr;
	}
}
			...
			((mPointSize <= aFontLayer->mMaxPointSize) || (aFontLayer->mMaxPointSize == -1)))
		{
			...
			if (active)
			{
				...
				if ((mScale == 1.0) && ((aFontLayer->mPointSize == 0) || (mPointSize == aFontLayer->mPointSize)))
				{
					...
					CharDataMap::iterator aCDItr = aFontLayer->mCharDataMap.begin();
					while (aCDItr != aFontLayer->mCharDataMap.end())
					{
						anActiveFontLayer->mScaledCharImageRects.insert(CharRectMap::value_type(aCDItr->first, aCDItr->second.mImageRect));
						++aCDItr;
					}
				}
				else
				{
					...
					while (aCDItr != aFontLayer->mCharDataMap.end())
					{
						Rect* anOrigRect = &aCDItr->second.mImageRect;
						...
						anActiveFontLayer->mScaledCharImageRects[aCDItr->first] = aScaledRect;
					...
					aCDItr = aFontLayer->mCharDataMap.begin();
					while (aCDItr != aFontLayer->mCharDataMap.end())
					{
						if ((Image*)aFontLayer->mImage != NULL)
						{
							g.DrawImage(aFontLayer->mImage, anActiveFontLayer->mScaledCharImageRects[aCDItr->first], aCDItr->second.mImageRect);
						}
						++aCDItr;
					}
...
// Symbian: removed std::mutex (single-threaded)
...
{
	// Symbian: single-threaded, no mutex needed
	...
	uint32_t aCurRawChar = 0;
	uint32_t aNextRawChar = 0;
	...
	while (aHasCur)
	{
		...
		uint32_t aChar = GetMappedChar(aCurRawChar);

		uint32_t aNextChar = aHasNext ? GetMappedChar(aNextRawChar) : 0;
...
uint32_t ImageFont::GetMappedChar(uint32_t theChar)
{
	CharMap::iterator anItr = mFontData->mCharMap.find(theChar);
