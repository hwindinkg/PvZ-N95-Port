...
// Forward declaration for DrawSeedPacket (defined in GridItem.cpp or elsewhere)
...
void Coin::CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
{
    ...
    mRenderOrder = Board::MakeRenderOrder(RENDER_LAYER_COIN_BANK, 0, 1);
    mScale = 1.0f;
    mUsableSeedType = SEED_NONE;
    mNeedsBouncyArrow = false;
    mHasBouncyArrow = false;
    mHitGround = false;
    mTimesDropped = 0;
    mPottedPlantSpec.InitializePottedPlant(SEED_NONE);

    if (IsSun())
    {
        float aPosX = mWidth * 0.5f;
        float aPosY = mHeight * 0.5f;
        Reanimation* aSunReanim = mApp->AddReanimation(0.0f, 0.0f, 0, REANIM_SUN);
        aSunReanim->SetPosition(mPosX + aPosX, mPosY + aPosY);
        aSunReanim->mLoopType = REANIM_LOOP;
        aSunReanim->mAnimRate = 6.0f;
        AttachReanim(mAttachmentID, aSunReanim, aPosX, aPosY);
    }
    else if (mType == COIN_SILVER)
    {
        mPosX -= 10.0f;
        mPosY -= 8.0f;
        ...
        float aPosX = 9.0f;
        float aPosY = 9.0f;
        Reanimation* aCoinReanim = mApp->AddReanimation(0.0f, 0.0f, 0, REANIM_COIN_SILVER);
        aCoinReanim->SetPosition(mPosX + aPosX, mPosY + aPosY);
        aCoinReanim->mLoopType = REANIM_LOOP;
    ...
    else if (mType == COIN_GOLD)
    {
        ...
        float aPosX = 9.0f;
        float aPosY = 9.0f;
        Reanimation* aCoinReanim = mApp->AddReanimation(0.0f, 0.0f, 0, REANIM_COIN_GOLD);
        aCoinReanim->SetPosition(mPosX + aPosX, mPosY + aPosY);
        aCoinReanim->mLoopType = REANIM_LOOP;
        aCoinReanim->mAnimTime = RandRangeFloat(0.0f, 0.99f);
        aCoinReanim->mAnimRate *= RandRangeFloat(0.6f, 1.0f);
    ...
    else if (mType == COIN_DIAMOND)
    {
        ...
        float aPosX = -3.0f;
        float aPosY = 4.0f;
        Reanimation* aCoinReanim = mApp->AddReanimation(0.0f, 0.0f, 0, REANIM_DIAMOND);
        aCoinReanim->SetPosition(mPosX + aPosX, mPosY + aPosY);
        aCoinReanim->mLoopType = REANIM_LOOP;
        aCoinReanim->mAnimTime = RandRangeFloat(0.0f, 0.99f);
    ...
    else if (mType == COIN_PRESENT_PLANT || mType == COIN_AWARD_PRESENT)
    {
        ...
        {
            SeedType aSeedType = static_cast<SeedType>(mApp->mZenGarden->PickRandomSeedType());
            mPottedPlantSpec.InitializePottedPlant(aSeedType);
        }
        else if (mBoard->mBackground == BACKGROUND_1_DAY)
        {
            SeedType aSeedList[] = {
                SEED_PEASHOOTER,
                SEED_SUNFLOWER,
        ...
        else if (mBoard->mBackground == BACKGROUND_2_NIGHT)
        {
            SeedType aSeedList[] = {
                SEED_PUFFSHROOM,
                SEED_SUNSHROOM,
        ...
        else if (mBoard->mBackground == BACKGROUND_3_POOL)
        {
            SeedType aSeedList[] = {
                SEED_LILYPAD,
                SEED_SQUASH,
        ...
        else if (mBoard->mBackground == BACKGROUND_4_FOG)
        {
            SeedType aSeedList[] = {
                SEED_SEASHROOM,
                SEED_PLANTERN,
        ...
        else if (mBoard->mBackground == BACKGROUND_5_ROOF)
        {
            SeedType aSeedList[] = {
                SEED_CABBAGEPULT,
                SEED_KERNELPULT,
    ...
    else if (mType == COIN_AWARD_MONEY_BAG || mType == COIN_AWARD_BAG_DIAMOND)
    {
        mWidth = IMAGE_MONEYBAG->GetCelWidth();
        mHeight = IMAGE_MONEYBAG->GetCelHeight();
        mRenderOrder = Board::MakeRenderOrder(RENDER_LAYER_ABOVE_UI, 0, 0);
    }
    else if (mType == COIN_CHOCOLATE || mType == COIN_AWARD_CHOCOLATE)
    {
    ...
    case COIN_MOTION_COIN:
        ...
        if (mType == COIN_AWARD_SILVER_SUNFLOWER || mType == COIN_AWARD_GOLD_SUNFLOWER)
        {
            mPosY -= 100.0f;
            mGroundY = static_cast<int32_t>(mPosY + 45.0f);
            if (mGroundY > 400)
            {
                mGroundY = 400;
            }
    ...
    case COIN_MOTION_FROM_PRESENT:
        ...
        mVelX = 0.0f;
        mGroundY = 600;
        mRenderOrder = Board::MakeRenderOrder(RENDER_LAYER_ABOVE_UI, 0, 1);
        break;
    ...
    case COIN_MOTION_FROM_BOSS:
        ...
        mVelX = -3.0f;
        mPosX = 750.0f;
        mPosY = mType == COIN_AWARD_SILVER_SUNFLOWER ? 130.0f : 245.0f;
        mGroundY = static_cast<int32_t>(mPosY + 40);
        break;
    ...
    if (mCoinMotion != COIN_MOTION_FROM_PRESENT)
    {
        PlayLaunchSound();
    }
}
...
{
    // This function didn't exist originally, but was kept since it's used in Board and
    // provides convenience when adding new coin types
    return theType == COIN_SILVER || theType == COIN_GOLD || theType == COIN_DIAMOND;
}
...
bool Coin::IsSun()
{
    return mType == COIN_SUN || mType == COIN_SMALLSUN || mType == COIN_LARGESUN;
}

bool Coin::IsPresentWithAdvice()
{
    return mType == COIN_PRESENT_MINIGAMES || mType == COIN_PRESENT_PUZZLE_MODE || mType == COIN_PRESENT_SURVIVAL_MODE;
}
...
{
    if (mApp->IsEndlessIZombie(mApp->mGameMode) || mApp->IsEndlessScaryPotter(mApp->mGameMode) || mType == COIN_NOTE || !IsLevelAward())
    {
        mFadeCount--;
        if (mFadeCount == 0)
        {
...
void Coin::UpdateFall()
{
    ...
    {
        mPosY += mVelY;
        if (mCoinMotion == COIN_MOTION_FROM_PLANT)
        {
            mVelY += 0.09f;
        }
        else if (mCoinMotion == COIN_MOTION_COIN || mCoinMotion == COIN_MOTION_FROM_BOSS)
        {
            mVelY += 0.15f;
        }

        mPosX += mVelX;
        if (mPosX > BOARD_WIDTH - mWidth && mCoinMotion != COIN_MOTION_FROM_BOSS)
        {
    ...
    else
    {
        if (mNeedsBouncyArrow && !mHasBouncyArrow)
        {
            ...
            else if (mType == COIN_AWARD_PRESENT || IsPresentWithAdvice())
            {
            ...
            if (mType == COIN_FINAL_SEED_PACKET)
            {
                aEffect = PARTICLE_SEED_PACKET;
            }
            else if (IsMoney())
            {
                aEffect = PARTICLE_COIN_PICKUP_ARROW;
            }
            else
            {
                aEffect = PARTICLE_AWARD_PICKUP_ARROW;
            }
        ...
        if (mApp->mGameMode != GAMEMODE_CHALLENGE_LAST_STAND || mBoard == NULL ||
            mBoard->mChallenge->mChallengeState == STATECHALLENGE_LAST_STAND_ONSLAUGHT)
        {
...
void Coin::UpdateCollected()
{
    ...
    else if (IsMoney())
    {
        ...
        if (mApp->GetDialog(static_cast<int>(DIALOG_STORE)))
        {
            aDestX = 662;
            aDestY = 546;
        }
        else if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZEN_GARDEN || mApp->mCrazyDaveState != CRAZY_DAVE_OFF)
        {
            aDestX = 442;
        }
    }
    ...
    else if (mType == COIN_AWARD_PRESENT || mType == COIN_PRESENT_PLANT)
    {
        mDisappearCounter++;
    ...
    if (IsLevelAward())
    {
        mScale = TodAnimateCurveFloat(0, 400, mDisappearCounter, 1.01f, 2.0f, CURVE_EASE_IN_OUT);
        mPosX = TodAnimateCurveFloat(0, 350, mDisappearCounter, mCollectX, aDestX, CURVE_EASE_OUT);
        mPosY = TodAnimateCurveFloat(0, 350, mDisappearCounter, mCollectY, aDestY, CURVE_EASE_OUT);
        return;
    }

    float aDeltaX = static_cast<float>(fabs(mPosX - aDestX));
    float aDeltaY = static_cast<float>(fabs(mPosY - aDestY));
    if (mPosX > aDestX)
    {
        mPosX -= aDeltaX / 21.0f;
    }
    ...
    if (IsPresentWithAdvice())
    {
        ...
        {
            if (!mBoard->mHelpDisplayed[ADVICE_UNLOCKED_MODE])
            {
                if (mType == COIN_PRESENT_MINIGAMES)
                {
                    // Note: theMessageStyle parameter in original is MESSAGE_STYLE_HINT_TALL_UNLOCKMESSAGE,
                    // in beta it's MESSAGE_STYLE_HINT_TALL_8SECONDS
                    mBoard->DisplayAdvice("[UNLOCKED_MINIGAMES]", MESSAGE_STYLE_HINT_TALL_UNLOCKMESSAGE, ADVICE_UNLOCKED_MODE);
                }
                else if (mType == COIN_PRESENT_PUZZLE_MODE)
                {
                    mBoard->DisplayAdvice("[UNLOCKED_PUZZLE_MODE]", MESSAGE_STYLE_HINT_TALL_UNLOCKMESSAGE, ADVICE_UNLOCKED_MODE);
                }
                else { // @Patoke: add case
                    mBoard->DisplayAdvice("[UNLOCKED_SURVIVAL_MODE]", MESSAGE_STYLE_HINT_TALL_UNLOCKMESSAGE, ADVICE_UNLOCKED_MODE);
                }
            }
            else if (mBoard->mHelpIndex != ADVICE_UNLOCKED_MODE || !mBoard->mAdvice->IsBeingDisplayed())
            {
                Die();
            }
        }
...
{
    mCoinAge++;
    if (mApp->mGameScene != SCENE_PLAYING && mApp->mGameScene != SCENE_AWARD && mBoard && !mBoard->mCutScene->ShouldRunUpsellBoard())
    {
        return;
    }
    ...
    if (mAttachmentID != ATTACHMENTID_NULL)
    {
        ...
        if ((!mHitGround || mIsBeingCollected) && (mType == COIN_SILVER || mType == COIN_GOLD))
        {
            AttachmentOverrideColor(mAttachmentID, Color(0, 0, 0, 0));  // Moving silver/gold coins use sprites, so hide attached animations
        }
    }
}
...
Color Coin::GetColor()
{
    if ((IsSun() || IsMoney()) && mIsBeingCollected)
    {
        ...
        return Color(255, 255, 255, static_cast<int>(aAlpha));
    }

    if (mFadeCount > 0)
    {
        int aAlpha = TodAnimateCurve(15, 0, mFadeCount, 255, 0, CURVE_LINEAR);
...
SeedType Coin::GetFinalSeedPacketType()
{
    ...
    {
        return static_cast<SeedType>(mApp->GetAwardSeedForLevel(mBoard->mLevel));
    }
...
void Coin::Draw(Graphics* g)
{
    ...
    if (mType == COIN_PRESENT_PLANT)
    {
        g->SetColorizeImages(true);
        g->DrawImage(IMAGE_AWARDPICKUPGLOW, mPosX - 50.0f, mPosY - 64.0f);
        g->SetColorizeImages(false);
    }
    if (mType == COIN_AWARD_PRESENT && mIsBeingCollected)
    {
        g->SetColorizeImages(true);
        g->DrawImage(IMAGE_AWARDPICKUPGLOW, mPosX - 50.0f, mPosY - 64.0f);
        g->SetColorizeImages(false);
    }
    if (mType == COIN_CHOCOLATE || mType == COIN_AWARD_CHOCOLATE)
    {
        g->SetColorizeImages(true);
    ...
    if ((mType == COIN_SILVER || mType == COIN_GOLD) && mHitGround && !mIsBeingCollected)
    {
        return;
    }
    ...
    else if (mType == COIN_FINAL_SEED_PACKET)
    {
        SeedType aSeedType = GetFinalSeedPacketType();
        g->SetScale(mScale, mScale, 0.0f, 0.0f);
        DrawSeedPacket(g, 0.5f * (mWidth - mScale * mWidth) + mPosX, 0.5f * (mHeight - mScale * mHeight) + mPosY, aSeedType, SEED_NONE, 0.0f, 255, true, false);
    ...
    else if (mType == COIN_AWARD_SILVER_SUNFLOWER)
    {
        aImage = IMAGE_SUNFLOWER_TROPHY;
        aOffsetX -= 5.0f;
        aDrawScale *= 0.6f;
    }
    else if (mType == COIN_AWARD_GOLD_SUNFLOWER)
    {
        aImage = IMAGE_SUNFLOWER_TROPHY;
        aImageCelCol = 1;
        aOffsetX -= 5.0f;
    ...
    else if (mType == COIN_USABLE_SEED_PACKET)
    {
        ...
        g->SetColorizeImages(true);
        DrawSeedPacket(g, static_cast<int>(mPosX), static_cast<int>(mPosY), mUsableSeedType, SEED_NONE, 0.0f, aGrayness, false, false);
        g->SetColorizeImages(false);
...
void Coin::FanOutCoins(CoinType theCoinType, int theNumCoins)
{
    ...
    for (int i = 0; i < theNumCoins; i++)
    {
        ...
        float aPosY = mPosY;
        Coin* aCoin = mBoard->AddCoin(static_cast<int>(aPosX), static_cast<int>(aPosY), theCoinType, COIN_MOTION_FROM_PRESENT);
...
void Coin::TryAutoCollectAfterLevelAward()
{
    ...
    if (IsMoney() && mCoinMotion != COIN_MOTION_FROM_PRESENT)
    {
    ...
    if (mType == COIN_PRESENT_PLANT || mType == COIN_CHOCOLATE || IsPresentWithAdvice())
    {
        aCanBeAutoCollected = true;
    }
...
void Coin::Collect()
{
    ...
    if (mType == COIN_PRESENT_PLANT || mType == COIN_AWARD_PRESENT)
    {
        TOD_ASSERT(mBoard);

        if (mApp->mZenGarden->IsZenGardenFull(false))
        {
            mBoard->DisplayAdvice("[DIALOG_ZEN_GARDEN_FULL]", MESSAGE_STYLE_HINT_FAST, ADVICE_NONE);
        }
        else
        {
            mBoard->mPottedPlantsCollected++;
            mBoard->DisplayAdvice("[ADVICE_FOUND_PLANT]", MESSAGE_STYLE_HINT_FAST, ADVICE_NONE);
            mApp->AddTodParticle(mPosX + 30.0f, mPosY + 30.0f, mRenderOrder + 1, PARTICLE_PRESENT_PICKUP);
        ...
        if (aIsEndlessAward)
        {
            AttachmentDetachCrossFadeParticleType(mAttachmentID, PARTICLE_AWARD_PICKUP_ARROW, NULL);
            mBoard->FadeOutLevel();
        }
    ...
    if (mType == COIN_PRESENT_PUZZLE_MODE)
    {
        TOD_ASSERT(mBoard);
    ...
    if (mType == COIN_PRESENT_SURVIVAL_MODE)
    {
        TOD_ASSERT(mBoard);
    ...
    if (mType == COIN_CHOCOLATE || mType == COIN_AWARD_CHOCOLATE)
    {
        ...
        if (mApp->mPlayerInfo->mPurchases[STORE_ITEM_CHOCOLATE] < PURCHASE_COUNT_OFFSET)
        {
            mBoard->DisplayAdvice("[ADVICE_FOUND_CHOCOLATE]", MESSAGE_STYLE_HINT_TALL_FAST, ADVICE_NONE);
            mApp->mPlayerInfo->mPurchases[STORE_ITEM_CHOCOLATE] = PURCHASE_COUNT_OFFSET + 1;
        }
        else
        {
            mApp->mPlayerInfo->mPurchases[STORE_ITEM_CHOCOLATE]++;
        }
    ...
    if (IsLevelAward())
    {
        if (aIsEndlessAward)
        {
            if (mType == COIN_AWARD_BAG_DIAMOND)
            {
                mApp->PlaySample(SOUND_DIAMOND);
                FanOutCoins(COIN_DIAMOND, 1);
                StartFade();
            }
            else if (mType == COIN_AWARD_MONEY_BAG)
            {
                mApp->PlayFoley(FOLEY_COIN);
                FanOutCoins(COIN_GOLD, 5);
                StartFade();
            }
        }
        else if (mApp->IsScaryPotterLevel())
        {
            ...
            {
                mApp->PlayFoley(FOLEY_COIN);
                FanOutCoins(COIN_GOLD, 2);
            }
        }
        else if (mApp->IsAdventureMode() && mBoard->mLevel == 50)
        {
            FanOutCoins(COIN_DIAMOND, 3);
        }
        else if (mType == COIN_AWARD_GOLD_SUNFLOWER)
        {
            FanOutCoins(COIN_DIAMOND, 5);
        }
        ...
        mApp->AddTodParticle(mPosX + 30.0f, mPosY + 30.0f, mRenderOrder + 1, PARTICLE_STARBURST);

        TOD_ASSERT(mBoard);

        mBoard->FadeOutLevel();
        AttachmentDetachCrossFadeParticleType(mAttachmentID, PARTICLE_SEED_PACKET, NULL);
        AttachmentDetachCrossFadeParticleType(mAttachmentID, PARTICLE_AWARD_PICKUP_ARROW, NULL);
        AttachmentDetachCrossFadeParticleType(mAttachmentID, PARTICLE_COIN_PICKUP_ARROW, NULL);
    ...
    if (mType == COIN_USABLE_SEED_PACKET)
    {
        ...
        mBoard->mCursorObject->mCursorType = CURSOR_TYPE_PLANT_FROM_USABLE_COIN;
    ...
    if (IsSun() && mBoard && !mBoard->HasConveyorBeltSeedBank())
    {
        for (int i = 0; i < mBoard->mSeedBank->mNumPackets; i++)
        {
            ...
            int aCost = mBoard->GetCurrentPlantCost(static_cast<SeedType>(aSeedPacket->mPacketType), static_cast<SeedType>(aSeedPacket->mImitaterType));
        ...
        {
            mRenderOrder = Board::MakeRenderOrder(RENDER_LAYER_ABOVE_UI, 0, 2);
        }
    }

    AttachmentDetachCrossFadeParticleType(mAttachmentID, PARTICLE_COIN_PICKUP_ARROW, NULL);
    if (mApp->IsFirstTimeAdventureMode() && mBoard && mBoard->mLevel == 11 && (mType == COIN_GOLD || mType == COIN_SILVER))
    {
        mBoard->DisplayAdvice("[ADVICE_CLICKED_ON_COIN]", MESSAGE_STYLE_HINT_FAST, ADVICE_CLICKED_ON_COIN);
    }
}

float Coin::GetSunScale()
{
    return mType == COIN_SMALLSUN ? 0.5f : mType == COIN_LARGESUN ? 2.0f : 1.0f;
}

int Coin::GetSunValue()
{
    return mType == COIN_SUN ? 25 : mType == COIN_SMALLSUN ? 15 : mType == COIN_LARGESUN ? 50 : 0;
}

int Coin::GetCoinValue(CoinType theCoinType)
{
    return theCoinType == COIN_SILVER ? 1 : theCoinType == COIN_GOLD ? 5 : theCoinType == COIN_DIAMOND ? 100 : 0;
}

void Coin::PlayLaunchSound()
{
    if (mType == COIN_DIAMOND ||
        mType == COIN_CHOCOLATE ||
        mType == COIN_AWARD_CHOCOLATE ||
        mType == COIN_PRESENT_PLANT ||
        mType == COIN_AWARD_PRESENT ||
        IsPresentWithAdvice())
    {
        mApp->PlayFoley(FOLEY_CHIME);
    }
}
...
void Coin::PlayGroundSound()
{
    if (mType == COIN_GOLD)
    {
        mApp->PlayFoley(FOLEY_MONEYFALLS);
        return;
    }
}
...
void Coin::MouseDown(int x, int y, int theClickCount)
{
    ...
    if (theClickCount >= 0 && !mIsBeingCollected)
    {
        ...
        {
            mBoard->DisplayAdvice("[ADVICE_CLICKED_ON_SUN]", MESSAGE_STYLE_TUTORIAL_LEVEL1_STAY, ADVICE_CLICKED_ON_SUN);
        }
    }
}
...
{
    return
        mType == COIN_FINAL_SEED_PACKET ||
        mType == COIN_TROPHY ||
        mType == COIN_AWARD_SILVER_SUNFLOWER ||
        mType == COIN_AWARD_GOLD_SUNFLOWER ||
        mType == COIN_SHOVEL ||
        mType == COIN_CARKEYS ||
        mType == COIN_ALMANAC ||
        mType == COIN_VASE ||
        ...
        mType == COIN_TACO ||
        mType == COIN_NOTE ||
        mType == COIN_AWARD_MONEY_BAG ||
        mType == COIN_AWARD_BAG_DIAMOND ||
        mType == COIN_AWARD_PRESENT ||
        mType == COIN_AWARD_CHOCOLATE;
}
